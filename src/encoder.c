...
static int FrameCodeI(Encoder * pEnc, Bitstream * bs);

static int FrameCodeP(Encoder * pEnc, Bitstream * bs);

static void FrameCodeB(Encoder * pEnc, FRAMEINFO * frame, Bitstream * bs);
...
static void simplify_time(int *inc, int *base) {
	/* common factor */
...
int enc_create(xvid_enc_create_t * create) {
	...
	if (create->num_zones > 0) {
		...
		pEnc->zones = xvid_malloc(sizeof(xvid_enc_zone_t) * pEnc->num_zones,
				CACHE_LINE);
		if (pEnc->zones == NULL)
			goto xvid_err_memory0;
		memcpy(pEnc->zones, create->zones,
				sizeof(xvid_enc_zone_t) * pEnc->num_zones);
	} else {
		pEnc->num_zones = 0;
		pEnc->zones = NULL;
	}
	...
	if (create->num_plugins > 0) {
		...
		pEnc->plugins = xvid_malloc(
				sizeof(xvid_enc_plugin_t) * pEnc->num_plugins, CACHE_LINE);
	...
	for (n = 0; n < pEnc->num_plugins; n++) {
		...
		if (create->plugins[n].func(NULL, XVID_PLG_CREATE, &pcreate,
				&pEnc->plugins[n].param) >= 0) {
	...
	if ((pEnc->mbParam.plugin_flags & XVID_REQDQUANTS)) {
		pEnc->temp_dquants = (int *) xvid_malloc(
				pEnc->mbParam.mb_width * pEnc->mbParam.mb_height * sizeof(int),
				CACHE_LINE);
	...
	if (pEnc->mbParam.plugin_flags & XVID_REQLAMBDA) {
		pEnc->temp_lambda = (float *) xvid_malloc(
				pEnc->mbParam.mb_width * pEnc->mbParam.mb_height * 6
						* sizeof(float), CACHE_LINE);
	...
	/* min/max quant */
	for (n = 0; n < 3; n++) {
		pEnc->mbParam.min_quant[n] =
				create->min_quant[n] > 0 ? create->min_quant[n] : 2;
		pEnc->mbParam.max_quant[n] =
				create->max_quant[n] > 0 ? create->max_quant[n] : 31;
	}
	...
	pEnc->mbParam.iMaxKeyInterval =
			create->max_key_interval <= 0 ?
					(10 * (int) pEnc->mbParam.fbase)
							/ (int) pEnc->mbParam.fincr :
					create->max_key_interval;
	...
	pEnc->current->mbs = xvid_malloc(
			sizeof(MACROBLOCK) * pEnc->mbParam.mb_width
					* pEnc->mbParam.mb_height, CACHE_LINE);
	pEnc->reference->mbs = xvid_malloc(
			sizeof(MACROBLOCK) * pEnc->mbParam.mb_width
					* pEnc->mbParam.mb_height, CACHE_LINE);
	...
	pEnc->mbParam.mpeg_quant_matrices = xvid_malloc(sizeof(uint16_t) * 64 * 8,
			CACHE_LINE);
	...
	if ((pEnc->mbParam.plugin_flags & XVID_REQORIGINAL)) {
		if (image_create(&pEnc->sOriginal, pEnc->mbParam.edged_width,
				pEnc->mbParam.edged_height) < 0)
			goto xvid_err_memory3;

		if (image_create(&pEnc->sOriginal2, pEnc->mbParam.edged_width,
				pEnc->mbParam.edged_height) < 0)
			goto xvid_err_memory3;
	}

	if (image_create(&pEnc->f_refh, pEnc->mbParam.edged_width,
			pEnc->mbParam.edged_height) < 0)
		goto xvid_err_memory3;
	if (image_create(&pEnc->f_refv, pEnc->mbParam.edged_width,
			pEnc->mbParam.edged_height) < 0)
		goto xvid_err_memory3;
	if (image_create(&pEnc->f_refhv, pEnc->mbParam.edged_width,
			pEnc->mbParam.edged_height) < 0)
		goto xvid_err_memory3;

	if (image_create(&pEnc->current->image, pEnc->mbParam.edged_width,
			pEnc->mbParam.edged_height) < 0)
		goto xvid_err_memory3;
	if (image_create(&pEnc->reference->image, pEnc->mbParam.edged_width,
			pEnc->mbParam.edged_height) < 0)
		goto xvid_err_memory3;
	if (image_create(&pEnc->vInterH, pEnc->mbParam.edged_width,
			pEnc->mbParam.edged_height) < 0)
		goto xvid_err_memory3;
	if (image_create(&pEnc->vInterV, pEnc->mbParam.edged_width,
			pEnc->mbParam.edged_height) < 0)
		goto xvid_err_memory3;
	if (image_create(&pEnc->vInterHV, pEnc->mbParam.edged_width,
			pEnc->mbParam.edged_height) < 0)
		goto xvid_err_memory3;

	/* Create full bitplane for GMC, this might be wasteful */
	if (image_create(&pEnc->vGMC, pEnc->mbParam.edged_width,
			pEnc->mbParam.edged_height) < 0)
		goto xvid_err_memory3;
	...
	if (pEnc->mbParam.max_bframes > 0) {
		...
		pEnc->bframes = xvid_malloc(
				pEnc->mbParam.max_bframes * sizeof(FRAMEINFO *), CACHE_LINE);
		...
		for (n = 0; n < pEnc->mbParam.max_bframes; n++) {
			...
			pEnc->bframes[n]->mbs = xvid_malloc(
					sizeof(MACROBLOCK) * pEnc->mbParam.mb_width
							* pEnc->mbParam.mb_height, CACHE_LINE);
	...
	pEnc->queue = xvid_malloc(
			(pEnc->mbParam.max_bframes + 1) * sizeof(QUEUEINFO), CACHE_LINE);
	...
	for (n = 0; n < pEnc->mbParam.max_bframes + 1; n++) {
		if (image_create(&pEnc->queue[n].image, pEnc->mbParam.edged_width,
				pEnc->mbParam.edged_height) < 0)
	...
	/* slices */
	pEnc->num_slices = MIN(MAX(1, create->num_slices),
			(int ) pEnc->mbParam.mb_height);
	...
	if (create->num_threads > 0) {
		...
		int rows_per_thread = (pEnc->mbParam.mb_height + threads_per_slice - 1)
				/ threads_per_slice;
		...
		pEnc->smpData[0].tmp_buffer = xvid_malloc(
				16 * pEnc->mbParam.edged_width * pEnc->mbParam.mb_height
						* sizeof(uint8_t), CACHE_LINE);
		...
		for (n = 0; n < t; n++) {
			...
			pEnc->smpData[n].complete_count_self = xvid_malloc(
					rows_per_thread * sizeof(int), CACHE_LINE);

			if (!pEnc->smpData[n].complete_count_self)
				goto xvid_err_nosmp;

			if (n > 0 && n < s) {
				pEnc->smpData[n].bs = (Bitstream *) xvid_malloc(
						sizeof(Bitstream), CACHE_LINE);
				if (!pEnc->smpData[n].bs)
					goto xvid_err_nosmp;

				pEnc->smpData[n].sStat = (Statistics *) xvid_malloc(
						sizeof(Statistics), CACHE_LINE);
				if (!pEnc->smpData[n].sStat)
					goto xvid_err_nosmp;

				pEnc->smpData[n].tmp_buffer = pEnc->smpData[0].tmp_buffer
						+ 16
								* (((n - 1) * pEnc->mbParam.edged_width
										* pEnc->mbParam.mb_height) / s);
				BitstreamInit(pEnc->smpData[n].bs, pEnc->smpData[n].tmp_buffer,
						0);
			}
	...
	xvid_err_memory2a: xvid_free(pEnc->mbParam.mpeg_quant_matrices);

	xvid_err_memory2: xvid_free(pEnc->current->mbs);
	xvid_free(pEnc->reference->mbs);

	xvid_err_memory1: xvid_free(pEnc->current);
	xvid_free(pEnc->reference);

	xvid_err_memory1a: if ((pEnc->mbParam.plugin_flags & XVID_REQDQUANTS)) {
	...
	xvid_err_memory0: for (n = 0; n < pEnc->num_plugins; n++) {
		if (pEnc->plugins[n].func) {
			pEnc->plugins[n].func(pEnc->plugins[n].param, XVID_PLG_DESTROY,
			NULL, NULL);
		}
	}
	...
	if (pEnc->num_plugins > 0) {
		...
		for (i = 0; i < pEnc->num_plugins; i++) {
			if (pEnc->plugins[i].func) {
				pEnc->plugins[i].func(pEnc->plugins[i].param, XVID_PLG_DESTROY,
						&pdestroy, NULL);
			}
		}
		xvid_free(pEnc->plugins);
...
static void call_plugins(Encoder * pEnc, FRAMEINFO * frame, IMAGE * original,
	...
	/* find zone */
	for (i = 0; i < pEnc->num_zones && pEnc->zones[i].frame <= frame->frame_num;
			i++)
		;
	...
	if (opt == XVID_PLG_BEFORE) {
		...
		if ((pEnc->mbParam.plugin_flags & XVID_REQDQUANTS)) {
			...
			memset(data.dquant, 0,
					data.mb_width * data.mb_height * sizeof(int));
		}
		...
		if ((frame->vol_flags & XVID_VOL_EXTRASTATS)
			...
			data.sse_y = plane_sse(original->y, frame->image.y,
					pEnc->mbParam.edged_width, pEnc->mbParam.width,
					pEnc->mbParam.height);

			data.sse_u = plane_sse(original->u, frame->image.u,
					pEnc->mbParam.edged_width / 2, pEnc->mbParam.width / 2,
					pEnc->mbParam.height / 2);

			data.sse_v = plane_sse(original->v, frame->image.v,
					pEnc->mbParam.edged_width / 2, pEnc->mbParam.width / 2,
					pEnc->mbParam.height / 2);
		}
		...
		if ((pEnc->mbParam.plugin_flags & XVID_REQDQUANTS)) {
			...
			for (j = 0; j < pEnc->mbParam.mb_height; j++)
				for (i = 0; i < pEnc->mbParam.mb_width; i++) {
					data.dquant[j * data.dquant_stride + i] = frame->mbs[j
							* pEnc->mbParam.mb_width + i].dquant;
				}
		}
	...
	for (i = 0; i < (unsigned int) pEnc->num_plugins; i++) {
		emms();
		if (pEnc->plugins[i].func) {
			if (pEnc->plugins[i].func(pEnc->plugins[i].param, opt, &data, NULL)
					< 0) {
				continue;
			}
		}
	...
	if (opt == XVID_PLG_BEFORE) {
		...
		*quant = data.quant > 0 ? data.quant : 2; /* default */
		...
		if ((pEnc->mbParam.plugin_flags & XVID_REQDQUANTS)) {
			for (j = 0; j < pEnc->mbParam.mb_height; j++)
				for (i = 0; i < pEnc->mbParam.mb_width; i++) {
					frame->mbs[j * pEnc->mbParam.mb_width + i].dquant =
							data.dquant[j * data.mb_width + i];
				}
		} else {
			for (j = 0; j < pEnc->mbParam.mb_height; j++)
				for (i = 0; i < pEnc->mbParam.mb_width; i++) {
		...
		if (pEnc->mbParam.plugin_flags & XVID_REQLAMBDA) {
			...
			for (j = 0; j < pEnc->mbParam.mb_height; j++)
				for (i = 0; i < pEnc->mbParam.mb_width; i++)
					for (k = 0; k < 6; k++) {
						frame->mbs[j * pEnc->mbParam.mb_width + i].lambda[k] = 1
								<< LAMBDA_EXP;
					}
		}
...
static __inline void MBSetDquant(MACROBLOCK * pMB, int x, int y,
		MBParam * mbParam) {
	if (pMB->cbp == 0) {
...
static __inline void set_timecodes(FRAMEINFO* pCur, FRAMEINFO *pRef,
		int32_t time_base) {

	pCur->ticks = (int32_t) pCur->stamp % time_base;
	pCur->seconds = ((int32_t) pCur->stamp / time_base)
			- ((int32_t) pRef->stamp / time_base);
...
static void simplify_par(int *par_width, int *par_height) {

	int _par_width = (!*par_width) ? 1 :
						(*par_width < 0) ? -*par_width : *par_width;
	int _par_height = (!*par_height) ? 1 :
						(*par_height < 0) ? -*par_height : *par_height;
...
int enc_encode(Encoder * pEnc, xvid_enc_frame_t * xFrame,
		xvid_enc_stats_t * stats) {
	xvid_enc_frame_t * frame;
	int type;
	Bitstream bs;
	static int cjy_num = 0;

	char cjy_type;
	if (XVID_VERSION_MAJOR(xFrame->version) != 1
	...
	if (xFrame->input.csp != XVID_CSP_NULL) {
		...
		if (image_input(&q->image, pEnc->mbParam.width, pEnc->mbParam.height,
				...
				xFrame->input.stride, xFrame->input.csp,
		...
		if ((xFrame->vop_flags & XVID_VOP_CHROMAOPT)) {
			image_chroma_optimize(&q->image, pEnc->mbParam.width,
					pEnc->mbParam.height, pEnc->mbParam.edged_width);
		}
		...
		if (xFrame->quant_inter_matrix) {
			memcpy(q->quant_inter_matrix, xFrame->quant_inter_matrix,
					64 * sizeof(unsigned char));
			q->frame.quant_inter_matrix = q->quant_inter_matrix;
		}

		pEnc->queue_tail = (pEnc->queue_tail + 1)
				% (pEnc->mbParam.max_bframes + 1);
		pEnc->queue_size++;
	}
	...
	if (pEnc->flush_bframes) {
		if (pEnc->bframenum_head < pEnc->bframenum_tail) {
			...
			DPRINTF(XVID_DEBUG_DEBUG,
					"*** BFRAME (flush) bf: head=%i tail=%i   queue: head=%i tail=%i size=%i\n",
					pEnc->bframenum_head, pEnc->bframenum_tail,
					pEnc->queue_head, pEnc->queue_tail, pEnc->queue_size);

			if ((pEnc->mbParam.plugin_flags & XVID_REQORIGINAL)) {
				image_copy(&pEnc->sOriginal2,
						&pEnc->bframes[pEnc->bframenum_head]->image,
						pEnc->mbParam.edged_width, pEnc->mbParam.height);
			}

			FrameCodeB(pEnc, pEnc->bframes[pEnc->bframenum_head], &bs);
			call_plugins(pEnc, pEnc->bframes[pEnc->bframenum_head],
					&pEnc->sOriginal2, XVID_PLG_AFTER, NULL, NULL, stats);
			pEnc->bframenum_head++;
	...
	if (pEnc->queue_size == 0) /* empty */
	{
		if (xFrame->input.csp == XVID_CSP_NULL) /* no futher input */
		{
			...
			if (!(pEnc->mbParam.global_flags & XVID_GLOBAL_PACKED)
				...
				call_plugins(pEnc, pEnc->current, &pEnc->sOriginal,
			...
			if (pEnc->bframenum_tail > 0) {
				...
				pEnc->bframenum_tail--;
				SWAP(FRAMEINFO*, pEnc->current,
						pEnc->bframes[pEnc->bframenum_tail]);

				/* convert B-VOP to P-VOP */
				pEnc->current->quant = 100 * pEnc->current->quant
						- pEnc->mbParam.bquant_offset;
		...
		goto done;
		/* nothing to encode yet; encoder lag */
	}
	...
	pEnc->current->fincr =
			pEnc->mbParam.fincr > 0 ? pEnc->mbParam.fincr : frame->fincr;
	inc_frame_num(pEnc);
	...
	if ((xFrame->vop_flags & XVID_VOP_CHROMAOPT)) {
		image_chroma_optimize(&pEnc->current->image, pEnc->mbParam.width,
				pEnc->mbParam.height, pEnc->mbParam.edged_width);
	}
	...
	call_plugins(pEnc, pEnc->current, NULL, XVID_PLG_BEFORE, &type,
			(int*) &pEnc->current->quant, stats);
	...
	if (type > 0) { /* XVID_TYPE_?VOP */
		...
		if (pEnc->iFrameNum == 0
			...
			type = MEanalysis(&pEnc->reference->image, pEnc->current,
					...
					xFrame->bframe_threshold,
					(pEnc->bframes) ?
							pEnc->bframes[pEnc->bframenum_head]->mbs : NULL);
		}
	}
	...
	if ((pEnc->current->vop_flags & XVID_VOP_DEBUG)) {
		image_printf(&pEnc->current->image, pEnc->mbParam.edged_width,
				pEnc->mbParam.height, 5, 5, "%d  st:%lld  if:%d",
				pEnc->current->frame_num, pEnc->current->stamp,
				pEnc->iFrameNum);
	}
	...
	if (type == I_VOP)
		...
		cjy_type = 'S';
	if (cjy_xvid_folder == NULL)
		cjy_enc = stdout;
	else {
		if (cjy_enc != NULL && cjy_enc != stdout)
			fclose(cjy_enc);
		char *file = malloc(10 + strlen(cjy_xvid_folder));
		sprintf(file, "%s/%04d_%1c", cjy_xvid_folder, cjy_num, cjy_type);
		cjy_enc = fopen(file, "w");
		if (cjy_enc == NULL) {
	...
	if (type == B_VOP) {
		if ((pEnc->current->vop_flags & XVID_VOP_DEBUG)) {
			image_printf(&pEnc->current->image, pEnc->mbParam.edged_width,
					pEnc->mbParam.height, 5, 200, "BVOP");
		}

		if (frame->quant < 1) {
			pEnc->current->quant = ((((pEnc->reference->quant
					+ pEnc->current->quant) * pEnc->mbParam.bquant_ratio) / 2)
					+ pEnc->mbParam.bquant_offset) / 100;
		...
		DPRINTF(XVID_DEBUG_DEBUG,
				...
				pEnc->bframenum_head, pEnc->bframenum_tail, pEnc->queue_head,
				pEnc->queue_tail, pEnc->queue_size, pEnc->current->quant);
	...
	DPRINTF(XVID_DEBUG_DEBUG,
			...
			pEnc->bframenum_head, pEnc->bframenum_tail, pEnc->queue_head,
			pEnc->queue_tail, pEnc->queue_size);
	...
	if (type == I_VOP && (pEnc->mbParam.global_flags & XVID_GLOBAL_CLOSED_GOP)
			&& pEnc->bframenum_tail > 0) {
		...
		pEnc->queue_head = (pEnc->queue_head + (pEnc->mbParam.max_bframes + 1)
				- 1) % (pEnc->mbParam.max_bframes + 1);
		pEnc->queue_size++;
		image_swap(&pEnc->current->image, &pEnc->queue[pEnc->queue_head].image);
	...
	if (type == I_VOP) {
		...
		if ((pEnc->current->vop_flags & XVID_VOP_DEBUG)) {
			image_printf(&pEnc->current->image, pEnc->mbParam.edged_width,
					pEnc->mbParam.height, 5, 200, "IVOP");
		}
		...
		if ((pEnc->mbParam.vol_flags & XVID_VOL_MPEGQUANT)) {
			if (frame->quant_intra_matrix != NULL)
				set_intra_matrix(pEnc->mbParam.mpeg_quant_matrices,
						frame->quant_intra_matrix);
			if (frame->quant_inter_matrix != NULL)
				set_inter_matrix(pEnc->mbParam.mpeg_quant_matrices,
						frame->quant_inter_matrix);
		}

		/* prevent vol/vop misuse */

		if (!(pEnc->current->vol_flags & XVID_VOL_INTERLACING))
			pEnc->current->vop_flags &= ~(XVID_VOP_TOPFIELDFIRST
		...
		if ((pEnc->current->vop_flags & XVID_VOP_DEBUG)) {
			image_printf(&pEnc->current->image, pEnc->mbParam.edged_width,
					pEnc->mbParam.height, 5, 200, "PVOP");
		}
...
static __inline void CodeIntraMB(MACROBLOCK * pMB) {
	pMB->mode = MODE_INTRA;
...
static void SliceCodeI(SMPData *data) {
	...
	for (y = data->start_y; y < data->stop_y; y++) {
		int new_bound = mb_width
				* ((((y * num_slices) / mb_height) * mb_height
		...
		for (x = 0; x < mb_width; x++) {
			...
			MBTransQuantIntra(&pEnc->mbParam, current, pMB, x, y, dct_codes,
					qcoeff);
...
static __inline void SerializeBitstreams(Encoder *pEnc, FRAMEINFO *current,
		Bitstream *bs, int num_threads) {
	int k;
...
static int FrameCodeI(Encoder * pEnc, Bitstream * bs) {
	...
	for (k = 0; k < num_threads; k++) {
		...
		pEnc->smpData[k].stop_y =
				(((bound + add) * mb_height + (num_slices - 1)) / num_slices);
		pEnc->smpData[k].start_y = ((bound * mb_height + (num_slices - 1))
				/ num_slices);
	...
	pEnc->current->sStat.kblks = pEnc->mbParam.mb_width
...
static __inline void updateFcode(Statistics * sStat, Encoder * pEnc) {
	float fSigma;
	int iSearchRange;
...
static void SliceCodeP(SMPData *data) {
	Encoder *pEnc = (Encoder *) data->pEnc;
	Bitstream *bs = (Bitstream *) data->bs;
	...
	for (y = data->start_y; y < data->stop_y; y++) {
		...
		for (x = 0; x < mb_width; x++) {
			...
			if (pMB->mode == MODE_INTRA || pMB->mode == MODE_INTRA_Q) {
				CodeIntraMB(pMB);
				MBTransQuantIntra(pParam, current, pMB, x, y, dct_codes,
						qcoeff);
			...
			start_timer();
			MBMotionCompensation(pMB, x, y, &reference->image, &pEnc->vInterH,
					&pEnc->vInterV, &pEnc->vInterHV, &pEnc->vGMC,
					&current->image, dct_codes, pParam->width, pParam->height,
					pParam->edged_width,
					(current->vol_flags & XVID_VOL_QUARTERPEL),
					current->rounding_type, data->RefQ);
			...
			if (pMB->cbp || pMB->mvs[0].x || pMB->mvs[0].y || pMB->mvs[1].x
					|| pMB->mvs[1].y || pMB->mvs[2].x || pMB->mvs[2].y
			...
			if ((pMB->mode == MODE_NOT_CODED) || (skip_possible)) {
				...
				if (current->coding_type == P_VOP) { /* special rule for P-VOP's SKIP */
					for (k = pEnc->bframenum_head; k < pEnc->bframenum_tail;
							k++) {
						int iSAD;
						iSAD = sad16(
								reference->image.y
										+ 16 * y * pParam->edged_width + 16 * x,
								pEnc->bframes[k]->image.y
										+ 16 * y * pParam->edged_width + 16 * x,
								pParam->edged_width,
								BFRAME_SKIP_THRESHHOLD * pMB->quant);
						if (iSAD >= BFRAME_SKIP_THRESHHOLD * pMB->quant
								|| ((bound > 1)
										&& ((y * mb_width + x == bound)
												|| (y * mb_width + x
														== bound + 1)))) { /* Some third-party decoders have problems with coloc skip MB before or after
							...
							if (pParam->vol_flags & XVID_VOL_QUARTERPEL) {
								VECTOR predMV = get_qpmv2(current->mbs,
										pParam->mb_width, bound, x, y, 0);
								pMB->pmvs[0].x = -predMV.x;
								pMB->pmvs[0].y = -predMV.y;
							} else {
								VECTOR predMV = get_pmv2(current->mbs,
										pParam->mb_width, bound, x, y, 0);
								pMB->pmvs[0].x = -predMV.x;
								pMB->pmvs[0].y = -predMV.y;
							}
...
static int FrameCodeP(Encoder * pEnc, Bitstream * bs) {
	...
	if ((current->vop_flags & XVID_VOP_HALFPEL)) {
		if (reference->is_interpolated != current->rounding_type) {
			...
			image_interpolate(pRef->y, pEnc->vInterH.y, pEnc->vInterV.y,
					pEnc->vInterHV.y, pParam->edged_width, pParam->edged_height,
					(pParam->vol_flags & XVID_VOL_QUARTERPEL),
	...
	{
		int gmcval;
		current->warp = GlobalMotionEst(current->mbs, pParam, current,
				reference, &pEnc->vInterH, &pEnc->vInterV, &pEnc->vInterHV,
				num_slices);

		if (current->motion_flags & XVID_ME_GME_REFINE) {
			gmcval = GlobalMotionEstRefine(&current->warp, current->mbs, pParam,
					current, reference, &current->image, &reference->image,
					&pEnc->vInterH, &pEnc->vInterV, &pEnc->vInterHV);
		} else {
			gmcval = globalSAD(&current->warp, pParam, current->mbs, current,
					&reference->image, &current->image, pEnc->vGMC.y);
		}

		gmcval += /*current->quant*/2
				* (int) (pParam->mb_width * pParam->mb_height);

		/* 1st '3': 3 warpoints, 2nd '3': 16th pel res (2<<3) */
		generate_GMCparameters(3, 3, &current->warp, pParam->width,
				pParam->height, &current->new_gmc_data);

		if ((gmcval < 0)
				&& ((current->warp.duv[1].x != 0)
						|| (current->warp.duv[1].y != 0)
						|| (current->warp.duv[2].x != 0)
						|| (current->warp.duv[2].y != 0))) {
			current->coding_type = S_VOP;

			generate_GMCimage(&current->new_gmc_data, &reference->image,
					pParam->mb_width, pParam->mb_height, pParam->edged_width,
					pParam->edged_width / 2, pParam->m_fcode,
					((pParam->vol_flags & XVID_VOL_QUARTERPEL) ? 1 : 0), 0,
...
#endif
	{
		...
		MotionEstimation(&pEnc->mbParam, current, reference, &pEnc->vInterH,
				&pEnc->vInterV, &pEnc->vInterHV, &pEnc->vGMC, 256 * 4096,
				num_slices);
	...
	for (k = 0; k < num_threads; k++) {
		...
		pEnc->smpData[k].start_y = ((bound * mb_height + (num_slices - 1))
				/ num_slices);
		pEnc->smpData[k].RefQ = ((k & 1) ? pEnc->vInterV.u : pEnc->vInterV.v)
				+ 16 * (k >> 1) * pParam->edged_width;

		bound += add;

		if (k > 0) {
			pEnc->smpData[k].sStat->iTextBits = pEnc->smpData[k].sStat->kblks =
					pEnc->smpData[k].sStat->mblks =
							pEnc->smpData[k].sStat->ublks =
	...
	if (current->sStat.kblks + current->sStat.mblks
		...
		image_copy(&current->image, &reference->image, pParam->edged_width,
				pParam->height);
		memcpy(current->mbs, reference->mbs,
				sizeof(MACROBLOCK) * mb_width * mb_height);
		coded = 0;
...
static void SliceCodeB(SMPData *data) {
	Encoder *pEnc = (Encoder *) data->pEnc;
	Bitstream *bs = (Bitstream *) data->bs;
		...
		for (x = start_x; x < stop_x; x++) {
			...
			if (mb->mode == MODE_NOT_CODED) {
				if (pParam->plugin_flags & XVID_REQORIGINAL) {
					MBMotionCompensation(mb, x, y, f_ref, NULL, f_ref, NULL,
					NULL, &frame->image,
			...
			if (mb->cbp != 0 || pParam->plugin_flags & XVID_REQORIGINAL) {
				...
				MBMotionCompensationBVOP(pParam, mb, x, y, &frame->image, f_ref,
						&pEnc->f_refh, &pEnc->f_refv, &pEnc->f_refhv, b_ref,
						&pEnc->vInterH, &pEnc->vInterV, &pEnc->vInterHV,
						dct_codes, data->RefQ);

				mb->cbp = MBTransQuantInterBVOP(pParam, frame, mb, x, y,
						dct_codes, qcoeff);
			}
			...
			if (mb->mode == MODE_DIRECT
					&& (mb->cbp | mb->pmvs[3].x | mb->pmvs[3].y) == 0)
				mb->mode = MODE_DIRECT_NONE_MV; /* skipped */
			else if (frame->vop_flags & XVID_VOP_GREYSCALE)
...
#define BFRAME_DEBUG  	if (!first && fp){ \
	...
	if (pEnc->reference->is_interpolated != 0) {
		start_timer();
		image_interpolate(f_ref->y, pEnc->f_refh.y, pEnc->f_refv.y,
				pEnc->f_refhv.y, pEnc->mbParam.edged_width,
				pEnc->mbParam.edged_height,
	...
	if (pEnc->current->is_interpolated != 0) {
		start_timer();
		image_interpolate(b_ref->y, pEnc->vInterH.y, pEnc->vInterV.y,
				pEnc->vInterHV.y, pEnc->mbParam.edged_width,
				pEnc->mbParam.edged_height,
				(pEnc->mbParam.vol_flags & XVID_VOL_QUARTERPEL), 0);
		stop_inter_timer();
	...
	if ((frame->vop_flags & XVID_VOP_RD_PSNRHVSM)
			&& (frame->vop_flags & XVID_VOP_RD_BVOP)) {
		image_block_variance(&frame->image, pEnc->mbParam.edged_width,
				frame->mbs, pEnc->mbParam.mb_width, pEnc->mbParam.mb_height);
	}
...
#endif 
	{
		...
		MotionEstimationBVOP(&pEnc->mbParam, frame,
				...
				pEnc->reference->mbs, f_ref, &pEnc->f_refh, &pEnc->f_refv,
				&pEnc->f_refhv, pEnc->current, b_ref, &pEnc->vInterH,
				&pEnc->vInterV, &pEnc->vInterHV, pEnc->num_slices);
	}
	stop_motion_timer();
