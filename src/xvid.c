...
int xvid_gbl_init(xvid_gbl_init_t * init) {
	...
	cpu_flags =
			(init->cpu_flags & XVID_CPU_FORCE) ?
					init->cpu_flags : detect_cpu_flags();
...
static int xvid_gbl_convert(xvid_gbl_convert_t* convert) {
	...
	case XVID_CSP_YV12:
		...
		img.v = (uint8_t*) convert->input.plane[0]
				+ convert->input.stride[0] * height;
		img.u = (uint8_t*) convert->input.plane[0]
				+ convert->input.stride[0] * height
...
int xvid_encore(void *handle, int opt, void *param1, void *param2) {
	...
	case XVID_ENC_ENCODE:
		...
		return enc_encode((Encoder *) handle, (xvid_enc_frame_t *) param1,
